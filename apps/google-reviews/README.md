...
A full-stack app for monitoring Google reviews and automating replies. Built for FlowCraft small-business automation.
...
```bash
cd apps/google-reviews/backend
...
```bash
cd apps/google-reviews/frontend
npm install
npm run dev
```
...
- Self-hosted; no built-in authentication (same pattern as other FlowCraft apps)
