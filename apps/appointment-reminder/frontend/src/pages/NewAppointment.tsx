...
export default function NewAppointment() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Failed to save. Please check the backend is running.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const fields: { label: string; key: keyof FormState; type: string; placeholder: string }[] = [
    { label: "Title *", key: "title", type: "text", placeholder: "e.g. Dental Checkup" },
    { label: "Client Name *", key: "client_name", type: "text", placeholder: "Full name" },
    { label: "Client Email *", key: "client_email", type: "email", placeholder: "client@email.com" },
    { label: "Client Phone", key: "client_phone", type: "tel", placeholder: "+1 555 000 0000" },
  ];

  return (
    <div className="p-8 max-w-2xl">
      <h2 className="text-2xl font-bold text-white mb-6">
        {id ? "Edit" : "New"} Appointment
      </h2>

      {error && (
        <div className="mb-4 px-4 py-2 bg-red-950 border border-red-700 rounded-lg text-sm text-red-300">
          {error}
        </div>
      )}
